...
// Reply via broadcast
func (d Discovered) Reply(payload []byte) {
	d.broadcast(payload)
}
...
func Discover(settings ...Settings) (discoveries []Discovered, err error) {
	...
	p.RLock()
	address := net.JoinHostPort(p.settings.MulticastAddress, "0")
	...
	// get interfaces
	ifaces, err := getLocalIFaces(allowSelf)
	if err != nil {
		return
	}

	// Open up a connection
	client, err := net.ListenPacket(fmt.Sprintf("udp%d", p.settings.IPVersion), address)
	if err != nil {
		return
	}
	defer client.Close()
	...
	var sender NetPacketConn
	if p.settings.IPVersion == IPv4 {
		sender = PacketConn4{ipv4.NewPacketConn(client)}
	} else {
		sender = PacketConn6{ipv6.NewPacketConn(client)}
	}

	addr := &net.UDPAddr{IP: group, Port: portNum}
	mcJoinGroup("s1", ifaces, sender, addr)
	defer mcLeaveGroup("s1", ifaces, sender, addr)

	listener, err := p.listen(func(data []byte) { broadcast(sender, data, ifaces, addr) })
	if err != nil {
		return
	}
	...
	for {
		...
		if !s.DisableBroadcast {
			...
			// write to multicast
			broadcast(sender, payload, ifaces, addr)
		}
...
func broadcast(p2 NetPacketConn, payload []byte, ifaces []net.Interface, dst net.Addr) {
	for i := range ifaces {
		...
		if _, errMulticast := p2.WriteTo(payload, dst); errMulticast != nil {
			continue
		}
	}
}
...
func (p *peerDiscovery) listen(broadcast Sender) (listener net.PacketConn, err error) {
	p.RLock()
	...
	// Open up a connection
	listener, err = net.ListenPacket(fmt.Sprintf("udp%d", p.settings.IPVersion), address)
	if err != nil {
		return
	}

	group := p.settings.multicastAddressNumbers
	var server NetPacketConn
	if p.settings.IPVersion == IPv4 {
		server = PacketConn4{ipv4.NewPacketConn(listener)}
	} else {
		server = PacketConn6{ipv6.NewPacketConn(listener)}
	}

	go func() {
		var err2 error
		addr := &net.UDPAddr{IP: group, Port: portNum}
		mcJoinGroup("s2", ifaces, server, addr)
		defer func() {
			if err2 == nil || !errors.Is(err2, net.ErrClosed) {
				mcLeaveGroup("s2", ifaces, server, addr)
			}
		}()
		...
		for {
			...
			n, src, errRead = server.ReadFrom(buffer)
			if errRead != nil {
				err2 = errRead
				return
			}
...
func mcJoinGroup(scene string, ifaces []net.Interface, conn NetPacketConn, addr *net.UDPAddr) {
	for i := range ifaces {
		if err := conn.JoinGroup(&ifaces[i], addr); err != nil {
			log.Printf("[%s] join group failed: %s\n", scene, err.Error())
		}
	}
}

// multicast leave group
func mcLeaveGroup(scene string, ifaces []net.Interface, conn NetPacketConn, addr *net.UDPAddr) {
	for i := range ifaces {
		if err := conn.LeaveGroup(&ifaces[i], addr); err != nil {
			log.Printf("[%s] leave group failed: %s\n", scene, err.Error())
		}
	}
}
...
// getLocalIFaces returns the local multicast links
func getLocalIFaces(allowSelf bool) ([]net.Interface, error) {
	ifaces, err := net.Interfaces()
	if err != nil {
		return nil, err
	}
	...
	for _, iface := range ifaces {
		if iface.Flags&net.FlagUp == 0 {
			...
			continue // ignore point-to-point link
		} else if iface.Flags&net.FlagLoopback != 0 && !allowSelf {
			continue // is loopback but not allow self
		} else if iface.Flags&net.FlagMulticast == 0 {
			continue // not support multicast
		}
	...
	if len(available) == 0 {
		return nil, errors.New("no available ifaces")
	}
